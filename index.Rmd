...
The idea for this case study is to take information from known FitBit data and see if there are potential ways to guide the advertisement of products from Bellabeat, either to improve the experience of current users or to potentially find a space in the markets whose needs have not yet been met.
...
Data used for this case study can be found on [this Kaggle page](https://www.kaggle.com/datasets/arashnic/fitbit). Licensing information can be found [here](https://creativecommons.org/publicdomain/zero/1.0/)
...
sleepPerDay$SleepDay <- round_date(mdy_hms(sleepPerDay$SleepDay), "day")
...
```
First thing of note is that sleep tracking and heart rate monitoring have significantly less users logged on them. There should still be enough to get some useful data out of them as long as there is enough daily tracked data in them.
...
While there is enough data from a lot of the users, there is also multiple users that only have a few logged times sleeping. When looking at the data for useful insights into behavior we don't want these affecting the data. It may also prove useful to see the trends for the users that only used this feature a couple times so I will be making a separate data frame that filters them out to keep the original data set intact.
...
sleepPerDayFiltered %>%
  ...
  summary()
```
1st and 3rd Quartile and Median data has shifted showing that the low data count was skewing the data lower. Most likely those users were not tracking their full sleeping habits and either occasional light sleeps were being tracked.
...
Similar to sleep tracking there are some who have much less data to be tracked. Their mean value does track significantly higher than the rest but this could be that they are only tracking their exercises which would still be valuable data to keep in the set for figuring out potential demographics so I won't be removing them but will be separating the data to try and pull further insights as we go.

## Adjusting data layouts

We're going to need to add some additional relevant data to most of the tables as well as grabbing the average values from the heart rate monitoring to make the data easier to visualize.
...
First up is daily heart rate monitoring. This is for trying to find consistent days of high activity. Now we need another table for hourly tracking. This will require some extra vectors to keep track of more information than the daily.
...
So looking at overall heart rates across the days there are a couple of outlines but no significant trends to be seen. But looking at each user, the data becomes a bit clearer.
...
A few things stand out here. First off is that you can fairly clearly see that different users have different average heart rates. This could be differences in resting heart rate or in daily life differences such as having a more active job or overall lifestyle. The one's with fewer data points overall also either have only low or only high measurements indicating that either:
1. Partial users only utilize heart rate monitoring for exercise. Or,
2. Partial users tried daily monitoring and either turned it off or stopped using the device out of lack of interest

However, the majority of users are tracking enough day to day which suggests that the majority of users do wear their fitness trackers daily and like to use the heart tracking function.
...
So looking at the overall trends there isn't a early adoption drop off overall, but there are spikes and drop offs overall in the low participation users, suggesting that users are picking up and dropping the feature quickly, just not as soon as they get their device. To be sure we need to look at each user's daily trends individually.
...
So it does look like users are picking up and dropping sleep tracking fairly quickly. Most individual users that didn't use much sleep tracking tried it for a couple of days and then maybe a day or two after that, and then gave up entirely. This suggests that there is something they run into that makes them quit using the feature. This could be a couple of things.
...
This could an area where Ivy is poised to fill a need. The Ivy being a smaller device that easily attached to a multitude of other wearables means that it could handle the comfort factor quiet easily. However, there isn't enough data to sort this possibility out from the rest. Before moving forward with this as an advertising angle I would recommend further studies, but that will be covered later.

## Sleep tracking usage

Next up is the sleep data that we can pull information from. Two things to look out for with sleep quality is both how long, 6-8 hours depending on age and other factors, but also sleep consistency. A range of sleep times can mean that someone is getting poor quality sleep even if they are getting the right ammount the majority of the time.
...
Looking at the density spread you would want to see a close bell curve near that 6-8 hour mark, indicating that someone is consistently getting the right amount of sleep. Very few of the users here show that. Some have some very small spots, usually meaning naps, but a lot are spread flat meaning that they are sleeping a wide range of times.
...
Here we're looking at the overall usage time added together. There's a heavy bias to sedentary minutes because as we saw with the heart rate monitoring, the majority of users wear their device all day, and many even overnight. At this scale it's hard to see any differences in higher activity levels but if we look at overall usage as a day of the week there is something of note.
...
1. Users are less active around the house on the weekend and don't put on the device (we will explore this later)
2. Users don't wear the FitBit when they go out on the weekends. Possibly because it doesn't match the outfit they are wearing.

Fitness trackers and smart watches are as much a fashion piece as they are tech, and Ivy already presents itself as a fashion piece so focusing the advertising on "the device for everyday, even when out with friends." This data is leaning towards showing that the looks of the device matter as much to the user, sometimes even more given the situation, as it's functionality.
...
Here is just the time spent not sedentary overall by users. There is a higher amount of time spent very active early on and it tapers off after April 25th suggesting that users try exercising more before falling back into more routine habits, but there's not a lot of data to go on there. More interesting is when we look at activity time against weekdays/weekends like before.
...
Learning from the daily heatrate monitor, let's look at individuals first to see what their habits are
```{r}
heartRateHourly %>% 
...
You can quickly see which users take off the FitBit at night as well as the one who mostly uses it for exercise. Overall the majority of the week is spent with lower average heart rates, as we saw with the amount of sedentary time. There are a few different types of trends to look for with the higher heart rates.
1. Clusters around 5PM to 8PM, indicating regular exercise after getting out of work.
2. Spread over the afternoons into early mornings indicating regular use even when out late like on the weekends. Only 2 of note there.
3. Overall wide spread indicating a more active lifestyle overall or a physically active job.
